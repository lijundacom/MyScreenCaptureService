...
package com.example.ljd.myscreencaptureservice.rtspserver.video;
...
import com.example.ljd.myscreencaptureservice.MainActivity;
import com.example.ljd.myscreencaptureservice.rtspserver.MediaStream;
import com.example.ljd.myscreencaptureservice.rtspserver.Stream;
import com.example.ljd.myscreencaptureservice.rtspserver.exceptions.CameraInUseException;
import com.example.ljd.myscreencaptureservice.rtspserver.exceptions.ConfNotSupportedException;
import com.example.ljd.myscreencaptureservice.rtspserver.exceptions.InvalidSurfaceException;
import com.example.ljd.myscreencaptureservice.rtspserver.gl.SurfaceView;
import com.example.ljd.myscreencaptureservice.rtspserver.hw.EncoderDebugger;
import com.example.ljd.myscreencaptureservice.rtspserver.hw.NV21Convertor;
import com.example.ljd.myscreencaptureservice.rtspserver.rtp.H264Packetizer;
import com.example.ljd.myscreencaptureservice.rtspserver.rtp.MediaCodecInputStream;
...
//		}
	...
	protected void encodeWithMediaCodecMethod2() throws RuntimeException, IOException {
		...
		//mMediaCodec = MediaCodec.createByCodecName(debugger.getEncoderName());
		Log.v(TAG,"encodeWithMediaCodecMethod2() 的各种信息长，宽，屏幕密度："+mQuality.resX+" "+mQuality.resY+" "+mQuality.resScreenDensity);
		MediaFormat mediaFormat = MediaFormat.createVideoFormat("video/avc", mQuality.resX, mQuality.resY);
		mediaFormat.setInteger(MediaFormat.KEY_BIT_RATE, mQuality.bitrate);
		mediaFormat.setInteger(MediaFormat.KEY_FRAME_RATE, mQuality.framerate);	
		...
		//((SurfaceView)mSurfaceView).addMediaCodecSurface(surface);//替换为createVisualDisplay
		mMediaCodec.start();
	...
	private void virtualDisplay(Surface mSurface) {
        ...
        //mQuality.resScreenDensity = 20;
        Log.v(TAG,"virtualDisplay 的各种信息长，宽，屏幕密度："+mQuality.resX+" "+mQuality.resY+" "+mQuality.resScreenDensity);
        //mMediaProjection = MainActivity.mMediaProjection;
        if(mMediaProjection == null){
            Log.v(TAG, "private void virtualDisplay: mMediaProjection == null");
        }
		mMediaProjection.createVirtualDisplay("屏幕捕捉",mQuality.resX, mQuality.resY,
				mQuality.resScreenDensity, DisplayManager.VIRTUAL_DISPLAY_FLAG_AUTO_MIRROR,
				mSurface,null,null);
